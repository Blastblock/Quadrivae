...
uint256 hashGenesisBlock("0xcd49456b7694a32c118588fd1a123d120aa259d6988c08ccd671a0b86a2ae437");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Quadrivae: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Quadrivae Signed Message:\n";
...
{
    // Quadrivae: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Quadrivae: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Quadrivae: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Quadrivae: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Quadrivae: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Quadrivae: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5fd5a6edb66ae3acad00d2dfe80832d81ef10eb8efee32a6f3c2932bcab18a82");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 05/Jun/2018 Manafort Is Accused of Attempted Witness Tampering";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b14e132f0b620f5d9e624ee949757051769f8bc17a183ca28b7e034d04c9e397f4de659e57e7fb4a3241c3e21a916a122a9c439f92bbd843ade1688a1fdd79f6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1528193476;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085708520;

        if (fTestNet)
        {
            block.nTime    = 1528193476;
            block.nNonce   = 385647291;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc4428ccf2472249d858cb27a7647ea77adcd3768cce408894e1c274bb813084f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc4, 0xb5, 0xdf }; // Quadrivae: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QuadrivaeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QuadrivaeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static QuadrivaeMiner(CWallet *pwallet)
{
    printf("QuadrivaeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("quadrivae-miner");
    ...
    try { loop {
        ...
        printf("Running QuadrivaeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("QuadrivaeMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&QuadrivaeMiner, pwallet));
}
